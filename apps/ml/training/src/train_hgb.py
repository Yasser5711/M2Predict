...
MODEL_PATH = ARTIFACTS_DIR / "dvf_price_m2_pipeline_hgb.joblib"
META_PATH = ARTIFACTS_DIR / "metadata_hgb.json"


def make_ohe():
    # compat scikit-learn versions
    try:
...
def main() -> None:
    pbar = tqdm(total=6, desc="train_hgb", unit="step")

    # 1 — Chargement données
    pbar.set_postfix_str("Chargement données")
    df = pd.read_parquet(DATA_PATH)
    # Create departement from code_postal
    df["departement"] = (
        df["code_postal"]
        .astype(str)
        .str.zfill(5)
