...
function CountingNumber({
	...
	const addThousandSep = React.useCallback(
		(str: string) => {
			if (!thousandSeparator) return str;
			const [intPart, fracPart] = str.split(decimalSeparator);
			const withSep = (intPart ?? "").replace(
				/\B(?=(\d{3})+(?!\d))/g,
				thousandSeparator,
			);
			return fracPart ? `${withSep}${decimalSeparator}${fracPart}` : withSep;
		},
	...
	React.useEffect(() => {
		const unsubscribe = springVal.on("change", (latest) => {
			if (localRef.current) {
				...
				localRef.current.textContent = addThousandSep(formatted);
			}
		});
		return () => unsubscribe();
	}, [
